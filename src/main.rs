...
impl StateMachine {
    ...
    /// of repeating pattern, written only for the scope of `tick`. 
    /// 
    /// This function returns `Some(_)` rather than `None` if and only if
...
fn main() {
    ...
    let source = match File::open(input_path) {
        Ok(file) => file.bytes(),
        
        Err(err) => {
            eprintln!("ERROR - could not open file `{input_path}` due to `{err}`");
            std::process::exit(1)
        }
    };
